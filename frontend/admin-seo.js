...
(function(){
  ...
  PGS.push("services","locations","programmatic","topics","authors","seosettings","schemaconfig","seofallbacks","repurpose","guides");
...
// ── GUIDES ──
window.ADMIN_GUIDES = {
  dashboard:{title:"Dashboard",icon:"\uD83D\uDCCA",steps:[
    {h:"Welcome to Multi-Directory",b:"Command center for all your directories. View stats and metrics across the platform."},
    {h:"Key Metrics",b:"Total directories, listings, users, and recent platform activity."},
    {h:"Quick Actions",b:"Create a new directory or jump to recent ones from here."}
  ]},
  directories:{title:"Directories",icon:"\uD83D\uDCC1",steps:[
    {h:"What is a Directory?",b:"A niche website (e.g. Miami Plumbers, Austin Dentists). Each directory is a standalone site with its own brand, domain, SEO settings, listings, and content."},
    {h:"Create a Directory",b:"Click + New Directory. Give it a name, slug, and brand colors. The directory gets its own subdomain or custom domain."},
    {h:"Clone a Directory",b:"Copy brand colors, SEO settings, and optionally listings for a head start on new directories."},
    {h:"Manage",b:"Click Manage to enter that directory admin panel. From there, you can add services, locations, listings, blog posts, and configure SEO."}
  ]},
  apikeys:{title:"API Keys",icon:"\uD83D\uDD11",steps:[
    {h:"API Keys Overview",b:"Manage API keys for programmatic access to the platform. Keys can be scoped per directory and revoked individually."}
  ]},
  legals:{title:"Legal Pages",icon:"\uD83D\uDCDC",steps:[
    {h:"Legal Pages",b:"Manage privacy policy, terms of service, and other legal pages per directory."}
  ]}
};

window.DIRECTORY_GUIDES = {
  listings:{title:"Listings",icon:"\uD83C\uDFE2",steps:[
    {h:"What are Listings?",b:"Business listings are the core of your directory. Each listing has name, address, phone, website, description, and categories."},
    {h:"Add Listings",b:"Manually add one at a time or import in bulk via CSV. Each gets its own detail page."},
    {h:"Categories",b:"Group listings by category (e.g. Residential, Commercial). Helps users filter and improves SEO."},
    {h:"Sister-City Linking",b:"When cloning a directory, sister-city links connect similar directories for cross-linking SEO value."}
  ]},
  services:{title:"Services",icon:"\uD83D\uDD27",steps:[
    {h:"What are Services?",b:"Define what your directory covers (e.g. Plumber, Electrician, Dentist). Each service x location becomes an SEO landing page."},
    {h:"Add Services",b:"Add individually or bulk import via CSV. Each service needs a name and slug."},
    {h:"Active vs Inactive",b:"Toggle services on/off without deleting. Inactive ones will not generate new pages."}
  ]},
  locations:{title:"Locations",icon:"\uD83D\uDCCD",steps:[
    {h:"What are Locations?",b:"Cities, neighborhoods, or regions your directory serves. Combined with services to generate location-specific pages."},
    {h:"CSV Import",b:"Bulk import from CSV: Name, Slug, State, Region. Great for covering 50+ cities quickly."},
    {h:"State and Region",b:"Metadata helps with SEO schema (geo-coordinates) and hierarchical navigation."}
  ]},
  programmatic:{title:"Landing Pages",icon:"\uD83D\uDCC4",steps:[
    {h:"What are Landing Pages?",b:"Auto-generated SEO pages for each Service x Location combination (e.g. Plumber in Miami). Each gets unique URL, meta tags, and content."},
    {h:"Generate Pages",b:"Select services and locations to create pages. Existing combos are skipped."},
    {h:"Edit and Publish",b:"Edit title, meta description, and content before publishing. Published pages appear on your directory public site."}
  ]},
  blog:{title:"Blog",icon:"\uD83D\uDCDD",steps:[
    {h:"Blog Overview",b:"Publish SEO-optimized blog posts per directory. Each post attracts organic traffic and builds topical authority."},
    {h:"Writing Posts",b:"Add title, content, excerpt, slug, and featured image. Assign to service, location, and author."},
    {h:"SEO Fields",b:"Set focus keyword, meta title/desc, canonical URL, robots meta, and schema type per post."},
    {h:"Publishing",b:"Draft posts remain private. Published posts are indexed and appear on your directory blog page."}
  ]},
  topics:{title:"Content Calendar",icon:"\uD83D\uDCC5",steps:[
    {h:"Overview",b:"Plan your content pipeline from idea to publication. Track every article status, keyword, service, and location."},
    {h:"Stages",b:"Suggested -> Scheduled -> In Progress -> In Review -> Published. Color-coded for quick scanning."},
    {h:"Suggest Topics",b:"Click Suggest Topics to generate content ideas from your services and locations."}
  ]},
  authors:{title:"Authors",icon:"\u270D\uFE0F",steps:[
    {h:"Author Profiles",b:"Manage contributors who write blog posts. Name, email, bio, avatar, and role."}
  ]},
  reviews:{title:"Reviews",icon:"\u2B50",steps:[
    {h:"Moderation Queue",b:"Approve, reject, or delete reviews. Only approved reviews count toward ratings."},
    {h:"Aggregate Rating",b:"Auto-calculated from approved reviews. Displays on listing cards and feeds into LocalBusiness schema."}
  ]},
  seosettings:{title:"SEO Settings",icon:"\u2699\uFE0F",steps:[
    {h:"Slug Pattern",b:"Control URL structure for landing pages: /{service}/{city} or /{city}/{service}."},
    {h:"Google Maps",b:"Enter Maps API key to show maps on listing pages. Hidden if no key configured."},
    {h:"AI Content",b:"Configure AI model (Claude/GPT), API key, and prompt template for auto-generated content."}
  ]},
  schemaconfig:{title:"Schema Config",icon:"\uD83D\uDD16",steps:[
    {h:"Schema Types",b:"Toggle LocalBusiness, FAQPage, Article, Product, Event, Service, Review, and more per directory."},
    {h:"No Code Needed",b:"All toggles stored in DB. Changes apply immediately without redeployment."}
  ]},
  seofallbacks:{title:"SEO Fallbacks",icon:"\uD83D\uDCCB",steps:[
    {h:"Fallback Templates",b:"Default meta templates using {service}, {city}, {site_name} variables for pages without custom values."}
  ]},
  repurpose:{title:"Repurpose",icon:"\uD83D\uDD04",steps:[
    {h:"FAQ Section",b:"Extract Q&A pairs from blog posts into FAQ schema format for rich snippets."},
    {h:"Social Media Pack",b:"Generate short, medium, and long versions for different platforms."}
  ]}
};

function loadGuides(){
  var el=dom("pageContent"),pa=dom("pageActions");pa.innerHTML="";
  var h="<h2 style=\"margin-bottom:8px\">Guides &amp; Walkthroughs</h2><p style=\"font-size:.9rem;color:var(--text-secondary);margin-bottom:24px\">Select a guide below. Guides are organized by level.</p>";
  h+="<h3 style=\"font-size:1rem;margin-bottom:12px;color:var(--teal)\">Platform Admin</h3><div class=\"card-grid\" style=\"margin-bottom:24px\">";
  h+=guideCard("dashboard","Dashboard","Command center overview","\uD83D\uDCCA");
  h+=guideCard("directories","Directories","Create, clone, and manage directories","\uD83D\uDCC1");
  h+=guideCard("apikeys","API Keys","Programmatic access management","\uD83D\uDD11");
  h+=guideCard("legals","Legal Pages","Privacy, terms, compliance","\uD83D\uDCDC");
  h+="</div>";
  h+="<h3 style=\"font-size:1rem;margin-bottom:12px;color:var(--teal)\">Directory Features</h3><div class=\"card-grid\">";
  h+=guideCard("listings","Listings","Business listings and categories","\uD83C\uDFE2");
  h+=guideCard("services","Services","Define services your directory covers","\uD83D\uDD27");
  h+=guideCard("locations","Locations","Cities and neighborhoods, CSV import","\uD83D\uDCCD");
  h+=guideCard("programmatic","Landing Pages","Auto-generated SEO pages","\uD83D\uDCC4");
  h+=guideCard("blog","Blog","SEO-optimized blog posts","\uD83D\uDCDD");
  h+=guideCard("topics","Content Calendar","Content pipeline management","\uD83D\uDCC5");
  h+=guideCard("authors","Authors","Author profiles","\u270D\uFE0F");
  h+=guideCard("reviews","Reviews","Moderation, ratings, schema","\u2B50");
  h+=guideCard("seosettings","SEO Settings","Slug patterns, Maps, AI config","\u2699\uFE0F");
  h+=guideCard("schemaconfig","Schema Config","Toggle structured data types","\uD83D\uDD16");
  h+=guideCard("seofallbacks","SEO Fallbacks","Default meta templates","\uD83D\uDCCB");
  h+=guideCard("repurpose","Repurpose","Convert posts to FAQ, social packs","\uD83D\uDD04");
  h+="</div>";
  el.innerHTML=h;
}
function guideCard(key,title,desc,icon){
  return "<div class=\"card\" style=\"cursor:pointer\" onclick=\"showGuideDetailed(\""+key+"\")"><div style=\"font-size:2rem;margin-bottom:8px\">"+icon+"</div><h3 style=\"font-size:1rem;margin-bottom:4px\">"+title+"</h3><p style=\"font-size:.8rem;color:var(--text-secondary)\">"+desc+"</p></div>";
}
function showGuideDetailed(key){
  var g=window.ADMIN_GUIDES[key]||window.DIRECTORY_GUIDES[key];
  if(!g){t("Guide not found","error");return;}
  var ov=modalOverlay("guideModal");
  var s="";
  for(var i=0;i<g.steps.length;i++){
    var st=g.steps[i];
    s+="<div style=\"display:flex;gap:12px;margin-bottom:16px;padding:12px;border:1px solid var(--border);border-radius:8px;background:var(--bg)\"><div style=\"width:28px;height:28px;border-radius:50%;background:var(--teal);color:#fff;display:flex;align-items:center;justify-content:center;font-size:.8rem;font-weight:700;flex-shrink:0\">"+(i+1)+"</div><div><strong style=\"font-size:.9rem\">"+st.h+"</strong><p style=\"font-size:.85rem;color:var(--text-secondary);margin-top:4px\">"+st.b+"</p></div></div>";
  }
  ov.innerHTML="<div class=\"modal\" style=\"width:600px;max-height:80vh;overflow-y:auto\"><div style=\"font-size:2.5rem;text-align:center;margin-bottom:8px\">"+g.icon+"</div><h2 style=\"text-align:center;margin-bottom:4px\">"+g.title+"</h2><p style=\"text-align:center;font-size:.85rem;color:var(--text-secondary);margin-bottom:20px\">"+g.steps.length+" steps</p>"+s+"<div class=\"modal-actions\"><button class=\"btn btn-secondary\" onclick=\"closeModal(\"guideModal\")\">Close</button></div></div>";
  docb(ov);
}

// ── DIRECTORY SWITCHING// ── DIRECTORY SWITCHING (hook into existing directories page) ──
