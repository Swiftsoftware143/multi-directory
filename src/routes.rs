...
pub fn create_router(s: AppState) -> Router {
    ...
    let all_routes = Router::new()
        ...
        .route(
            "/directories/:slug/render",
            get(directories::render_directory),
        )
        .route(
            "/directories/:slug/categories",
            get(directories::list_categories).post(directories::create_category),
        )
        .route(
            "/directories/:slug/categories/:category_id",
            put(directories::update_category).delete(directories::delete_category),
        )
        .route(
            "/directories/:slug/businesses",
            get(businesses::list_businesses).post(businesses::create_business),
        )
        .route(
            "/directories/:slug/businesses/suggestions",
            get(businesses::search_suggestions),
        )
        // Legacy /directory/:slug/businesses alias
        .route(
            "/directory/:slug/businesses",
            get(businesses::list_businesses),
        )
        .route(
            "/directories/:slug/businesses/:business_id",
            get(businesses::get_business)
                .put(businesses::update_business)
                .delete(businesses::delete_business),
        )
        .route(
            "/directories/:slug/loyalty/programs",
            get(loyalty_native::list_programs).post(loyalty_native::create_program),
        )
        .route(
            "/directories/:slug/loyalty/programs/:program_id",
            get(loyalty_native::get_program_handler)
                .put(loyalty_native::update_program)
                .delete(loyalty_native::delete_program),
        )
        .route(
            "/directories/:slug/loyalty/programs/:program_id/enroll",
            post(loyalty_native::enroll_member),
        )
        .route(
            "/directories/:slug/loyalty/programs/:program_id/checkin",
            post(loyalty_native::checkin),
        )
        .route(
            "/directories/:slug/loyalty/members/:visitor_account_id",
            get(loyalty_native::get_member),
        )
        .route(
            "/directories/:slug/loyalty/programs/:program_id/tiers",
            get(loyalty_native::list_tiers).post(loyalty_native::create_tier),
        )
        .route(
            "/directories/:slug/loyalty/programs/:program_id/tiers/:tier_id",
            delete(loyalty_native::delete_tier),
        )
        .route(
            "/directories/:slug/loyalty/programs/:program_id/rewards",
            get(loyalty_native::list_rewards).post(loyalty_native::create_reward),
        )
        .route(
            "/directories/:slug/loyalty/programs/:program_id/rewards/:reward_id",
            delete(loyalty_native::delete_reward),
        )
        .route(
            "/directories/:slug/loyalty/programs/:program_id/rewards/earn",
            post(loyalty_native::earn_reward),
        )
        .route(
            "/directories/:slug/loyalty/programs/:program_id/milestones",
        ...
        .route(
            "/networks/:slug/clear/treasury",
            get(clearinghouse::treasury_summary),
        )
        .route(
            "/networks/:slug/clear/ledgers",
            get(clearinghouse::business_ledgers),
        )
        .route(
            "/networks/:slug/clear/caps",
            get(clearinghouse::category_caps).put(clearinghouse::upsert_category_cap),
        )
        .route(
            "/networks/:slug/clear/expire",
            post(clearinghouse::expire_points),
        )
        .route(
        ...
        .route(
            ...
            get(newsletter::get_email_settings)
                .put(newsletter::upsert_email_settings)
                .delete(newsletter::delete_email_settings),
        )
        ...
        .route(
            "/directories/:slug/email-settings/test",
            post(newsletter::test_email_settings),
        )
        .route(
            "/directories/:slug/email-settings/test-send",
            post(newsletter::send_test_email),
        )
        .route(
            "/email-service/status",
            get(newsletter::email_service_status),
        )
        .route(
            "/directories/:slug/subscribers",
            get(newsletter::list_subscribers).post(newsletter::add_subscriber),
        )
        .route(
            "/directories/:slug/subscribers/import",
            post(newsletter::import_subscribers),
        )
        .route(
            "/directories/:slug/subscribers/:id/unsubscribe",
            post(newsletter::unsubscribe_subscriber),
        )
        .route(
        ...
        .route(
            "/blog-posts",
            get(blog::list_blog_posts).post(blog::create_blog_post),
        )
        .route(
            "/blog-posts/:id",
        ...
        .route(
            "/directories/:slug/blog-posts",
            get(blog::list_directory_blog_posts),
        )
        ...
        .route(
            "/directories/:slug/blog",
            get(blog::list_directory_blog_posts),
        )
        // Blog automation routes (Phase 5)
        .route(
            "/blog-templates",
            get(blog::list_templates).post(blog::create_template),
        )
        .route(
            "/blog-templates/:id",
            get(blog::get_template)
                .put(blog::update_template)
                .delete(blog::delete_template),
        )
        .route(
            "/blog-posts/ext",
            get(blog::list_blog_posts_ext).post(blog::create_blog_post_ext),
        )
        .route("/blog-posts/:id/ext", put(blog::update_blog_post_ext))
        .route(
            "/blog-posts/:id/publish",
            post(blog::publish_blog_post_handler),
        )
        .route("/blog-posts/scheduled", get(blog::list_scheduled_posts))
        .route("/blog/distribute", post(blog::distribute_blog_post))
        .route(
            "/blog/process-scheduled",
            post(blog::process_scheduled_posts_handler),
        )
        .route(
            "/directories/:slug/blog-posts/ext",
            get(blog::list_directory_blog_posts_ext),
        )
        .route(
            "/newsletters",
            get(newsletter::list_newsletters).post(newsletter::create_newsletter),
        )
        .route(
            "/newsletters/:id",
            get(newsletter::get_newsletter)
                .put(newsletter::update_newsletter)
                .delete(newsletter::delete_newsletter),
        )
        .route(
            "/newsletters/:id/generate",
            post(newsletter::generate_newsletter_content),
        )
        ...
        .route(
            "/blog-posts/:id/regenerate",
            post(blog_generator::regenerate_blog_post),
        )
        .route(
            "/blog-templates/:id/directories",
            get(blog_generator::get_template_directories)
                .post(blog_generator::set_template_directories),
        )
        ...
        .route(
            "/blog/decay/refresh/:id",
            post(blog_features::refresh_post_content),
        )
        .route(
            "/blog/internal-links/suggestions",
            get(blog_features::internal_link_suggestions),
        )
        .route(
            "/blog/internal-links/add",
            post(blog_features::add_internal_link),
        )
        .route(
            "/blog/internal-links/:post_id/:target_id",
            delete(blog_features::remove_internal_link),
        )
        .route("/blog/aeo/score/:id", post(blog_features::score_post_aeo))
        .route(
            "/blog/aeo/scan-all",
            post(blog_features::score_all_posts_aeo),
        )
        .route("/blog/aeo/report", get(blog_features::aeo_report))
        .route(
            "/blog/schema/generate/:id",
            post(blog_features::generate_schema_markup),
        )
        .route(
            "/blog/schema/generate-all",
            post(blog_features::generate_all_schema),
        )
        // Content Research & Strategy Engine
        .route(
            "/research/topics",
            get(content_research::list_topics).post(content_research::create_topic),
        )
        .route(
            "/research/topics/:id",
            put(content_research::update_topic).delete(content_research::delete_topic),
        )
        .route("/research/run", post(content_research::research_topic))
        .route(
            "/research/questions",
            get(content_research::get_research_questions),
        )
        .route(
            "/research/questions/:id/use-as-keyword",
            post(content_research::use_question_as_keyword),
        )
        .route(
            "/research/draft-post",
            post(content_research::draft_post_from_question),
        )
        .route("/research/bulk", post(content_research::bulk_research))
        .route(
            "/research/bulk/preview",
            post(content_research::bulk_research_preview),
        )
        .route(
            "/research/integrations",
            get(content_research::list_integrations).post(content_research::save_integration),
        )
        .route(
            "/research/integrations/:provider",
            delete(content_research::delete_integration),
        )
        .route(
            "/crm/contacts",
            get(crm::list_contacts).post(crm::create_contact),
        )
        .route(
            "/crm/contacts/:id",
            get(crm::get_contact)
                .put(crm::update_contact)
                .delete(crm::delete_contact),
        )
        .route("/crm/contacts/search", get(crm::search_contacts))
        .route(
            "/crm/pipelines",
            get(crm::list_pipelines).post(crm::create_pipeline),
        )
        .route(
            "/crm/pipelines/:id",
            get(crm::get_pipeline)
                .put(crm::update_pipeline)
                .delete(crm::delete_pipeline),
        )
        ...
        .route(
            ...
            get(crm::get_deal)
                .put(crm::update_deal)
                .delete(crm::delete_deal),
        )
        .route(
            "/directories/:slug/crm/stats",
            get(crm::directory_crm_stats),
        )
        .route(
            "/legal-pages",
            get(legal::list_legal_pages).post(legal::create_legal_page),
        )
        .route(
            "/legal-pages/:id",
            get(legal::get_legal_page)
                .put(legal::update_legal_page)
                .delete(legal::delete_legal_page),
        )
        ...
        .route(
            "/deals/redemptions/code/:code",
            get(deals::lookup_redemption),
        )
        .route("/directories/:slug/deals", get(deals::list_directory_deals))
        .route(
            "/directories/:slug/businesses/:business_id/deals",
            get(deals::list_business_deals),
        )
        .route(
            "/submissions",
        ...
        // ??? SEO routes
        .route(
            "/seo/:page_type/:page_id",
            get(seo::get_seo_meta).put(seo::update_seo_meta),
        )
        .route("/seo/sitemap-config", get(seo::list_all_sitemap_configs))
        .route(
            "/seo/sitemap-config/:directory_id",
        ...
        .route(
            "/search/config",
            get(search::list_search_configs).post(search::create_search_config),
        )
        .route(
            "/search/config/:directory_id",
            get(search::get_search_config).put(search::update_search_config),
        )
        ...
        .route(
            ...
            get(category_system::get_filter_options),
        )
        .route(
            "/businesses/:id/categories",
            get(category_system::get_business_categories)
                .put(category_system::set_business_categories)
                .post(category_system::set_business_categories),
        )
        .route(
            "/businesses/:id/categories/:category_id",
            delete(category_system::delete_business_category),
        )
        .route(
            "/businesses/bulk/categories",
            post(category_system::bulk_assign_categories),
        )
        .route(
            "/businesses/:id/category-requests",
            post(category_system::create_category_request),
        )
        .route(
            "/category-requests",
            get(category_system::list_category_requests),
        )
        .route(
            "/category-requests/:id/approve",
            post(category_system::approve_category_request),
        )
        .route(
            "/category-requests/:id/deny",
            post(category_system::deny_category_request),
        )
        // Community posts (BL27)
        .route(
            "/community/posts",
            get(blog::list_community_posts).post(blog::create_community_post),
        )
        .route(
            "/community/posts/:id",
            get(blog::get_blog_post)
                .put(blog::update_community_post)
                .delete(blog::delete_blog_post),
        )
        // Q&A Automation
        ...
        .route(
            "/integration-configs",
            get(blog_qa::list_configs).post(blog_qa::save_config),
        )
        .route(
            "/integration-configs/:provider",
            get(blog_qa::get_config).delete(blog_qa::delete_config),
        )
        // ??? Answer-First article generator + CoreSwift tenant setup
        .route(
            "/admin/articles/generate-answer-first",
            post(answer_first::generate_answer_first),
        )
        .route(
            "/admin/articles/suggest-competitors",
            post(answer_first::suggest_competitors),
        )
        .route(
            "/admin/core-swift/setup-tenant",
            post(answer_first::setup_core_swift_tenant),
        )
        .route(
            "/admin/core-swift/test-connection",
            post(answer_first::test_core_swift_connection),
        )
        ...
        .route(
            "/b2b/rfqs/:id/bids",
            get(rfq::list_bids).post(rfq::submit_bid),
        )
        ...
        .route(
            "/b2b/co-op/groups",
            get(coop::list_groups).post(coop::create_group),
        )
        ...
        .route(
            "/b2b/products/:id",
            get(b2b::get_product)
                .put(b2b::update_product)
                .delete(b2b::delete_product),
        )
        ...
        .route(
            "/b2b/messages",
            post(b2b::send_b2b_message).get(b2b::my_b2b_messages),
        )
        ...
        .route(
            "/b2b/notifications/:id/read",
            put(b2b::mark_notification_read),
        )
        .route("/b2b/notifications/read-all", put(b2b::mark_all_read))
        // Supplier Portal (back office)
        .route(
            "/supplier/profile",
            get(supplier::get_supplier_profile).put(supplier::update_supplier_profile),
        )
        .route(
            "/supplier/delivery",
            put(supplier::update_delivery_settings),
        )
        .route(
            "/supplier/featured-product",
            put(supplier::set_featured_product),
        )
        .route("/supplier/stats", get(supplier::supplier_stats))
        .route("/b2b/orders/:id/fulfill", put(supplier::fulfill_order))
        .route(
            "/b2b/orders/:id/review",
            post(supplier::supplier_review_buyer),
        )
        ...
        .route(
            ...
            post(scraper::scrape_google_places),
        )
        .route(
            "/scraper/populate-category",
            post(scraper::populate_category),
        )
        ...
        .route(
            ...
            get(articles_feed::articles_xml_feed),
        )
        .route(
            "/public/directories/:slug/news-sitemap.xml",
            get(blog_seo::news_sitemap),
        )
        .route(
            "/public/directories/:slug/blog/feed.xml",
            get(blog_seo::blog_rss_feed),
        )
        .route("/public/:slug", get(public::directory_data))
        .route("/public/:slug/:business_id", get(public::business_data))
        .route(
            "/landing-pages",
            get(public_pages::list_landing_pages).post(public_pages::create_landing_page),
        )
        .route(
            "/landing-pages/:id",
            get(public_pages::get_landing_page)
                .put(public_pages::update_landing_page)
                .delete(public_pages::delete_landing_page),
        )
        .route(
            "/landing-pages/:slug/publish",
            post(public_pages::toggle_publish),
        )
        .route(
            "/public-themes",
            get(public_pages::list_public_themes).post(public_pages::create_public_theme),
        )
        .route(
            "/public-themes/:id",
            get(public_pages::get_public_theme)
                .put(public_pages::update_public_theme)
                .delete(public_pages::delete_public_theme),
        )
        ...
        .route(
            "/directories/:slug/public-pages",
            get(public::list_directory_public_pages),
        )
        .route(
            "/public-pages/featured",
            get(public_pages::list_landing_pages),
        )
        .route(
            "/public-pages/:slug/publish",
            post(public_pages::toggle_publish),
        )
        ...
        .route(
            ...
            get(import_export::export_businesses),
        )
        .route(
            "/export/reviews/:directory_id",
            get(import_export::export_reviews),
        )
        .route(
        ...
        .route(
            "/export/templates/:id",
            get(import_export::get_export_template)
                .put(import_export::update_export_template)
                .delete(import_export::delete_export_template),
        )
        .route(
            "/export/templates/:id/run",
        ...
        .route(
            "/monetization/tiers/:id",
            get(monetization::get_tier)
                .put(monetization::update_tier)
                .delete(monetization::delete_tier),
        )
        .route(
            "/monetization/subscriptions",
            get(monetization::list_subscriptions).post(monetization::create_subscription),
        )
        .route(
            "/monetization/subscriptions/:id",
            get(monetization::get_subscription)
                .put(monetization::update_subscription)
                .delete(monetization::delete_subscription),
        )
        .route(
            "/monetization/ad-zones",
            get(monetization::list_ad_zones).post(monetization::create_ad_zone),
        )
        .route(
            "/monetization/ad-zones/:id",
            get(monetization::get_ad_zone)
                .put(monetization::update_ad_zone)
                .delete(monetization::delete_ad_zone),
        )
        ...
        .route(
            ...
            post(monetization::upgrade_subscription),
        )
        .route(
            "/subscriptions/downgrade",
            post(monetization::downgrade_subscription),
        )
        .route(
            "/subscriptions/features",
            get(monetization::check_feature_access),
        )
        .route(
            "/businesses/:id/subscription",
            get(monetization::business_subscription),
        )
        // (moved to category_system above)
        // ? Business self-serve ad submission (auth required)
        .route(
            "/businesses/:business_id/ads/submit",
            post(monetization::submit_business_ad),
        )
        .route(
            "/businesses/:business_id/ads",
            get(monetization::list_business_ads),
        )
        .route(
            "/businesses/:business_id/ads/earnings",
            get(monetization::get_business_ad_earnings),
        )
        .route(
            "/ad-zones",
            get(monetization::list_ad_zones).post(monetization::create_ad_zone),
        ...
        .route(
            "/sponsors",
            get(monetization::list_sponsors).post(monetization::create_sponsor),
        )
        .route(
            "/sponsors/:id",
            put(monetization::update_sponsor).delete(monetization::delete_sponsor),
        )
        .route(
            "/sponsors/:id/creatives",
            get(monetization::list_creatives).post(monetization::create_creative),
        )
        .route(
            "/creatives/:id",
            put(monetization::update_creative).delete(monetization::delete_creative),
        )
        .route(
            "/schedules",
            get(monetization::list_schedules).post(monetization::create_schedule),
        )
        .route(
            "/schedules/:id",
            put(monetization::update_schedule).delete(monetization::delete_schedule),
        )
        // .route("/ads/active/:directory_id", get(monetization::get_active_ads)) -- moved to public
        .route(
            "/earnings/:directory_id",
            get(monetization::get_earnings_summary),
        )
        .route("/approvals", get(monetization::list_approvals))
        .route("/approvals/:id/status", put(monetization::update_approval))
        .route(
            "/directories/:slug/ad-zones",
            get(monetization::directory_ad_zones),
        )
        // ??? Directory tier routes (Phase 3 Task 3)
        .route(
            "/monetization/directory-tiers",
            get(monetization::list_directory_tiers).post(monetization::create_directory_tier),
        )
        .route(
            "/monetization/directory-tiers/:id",
            get(monetization::get_directory_tier)
                .put(monetization::update_directory_tier)
                .delete(monetization::delete_directory_tier),
        )
        .route(
            "/monetization/directories/:slug/tier",
            get(monetization::directory_tier_by_slug),
        )
        // ??? Sponsored listing routes (Phase 3 Task 3)
        .route(
            "/monetization/sponsored-listings",
            get(monetization::list_sponsored_listings).post(monetization::create_sponsored_listing),
        )
        .route(
            "/monetization/sponsored-listings/:id",
            get(monetization::get_sponsored_listing)
                .put(monetization::update_sponsored_listing)
                .delete(monetization::delete_sponsored_listing),
        )
        .route(
            "/monetization/directories/:slug/sponsored-listings",
            get(monetization::directory_sponsored_listings),
        )
        // ??? Directory Notification routes (Phase 4)
        .route(
            "/monetization/notifications",
            get(monetization::list_notifications).post(monetization::create_notification),
        )
        .route(
            "/monetization/notifications/:id",
            put(monetization::update_notification).delete(monetization::delete_notification),
        )
        // ??? Call Tracking routes
        .route(
            "/call-logs",
            get(call_tracking::list_call_logs).post(call_tracking::create_call_log),
        )
        ...
        .route(
            "/places/autocomplete",
            get(data_company::places_autocomplete),
        )
        ...
        .route(
            "/verifications",
            get(data_company::list_verifications).post(data_company::create_verification),
        )
        .route(
            "/verifications/:id",
            get(data_company::get_verification).put(data_company::update_verification),
        )
        .route(
            "/businesses/:id/verifications",
            get(data_company::business_verifications),
        )
        ...
        .route(
            "/directories/:id/services",
            get(services_locations::list_services).post(services_locations::create_service),
        )
        .route(
            "/directories/:id/services/:svc_id",
            put(services_locations::update_service).delete(services_locations::delete_service),
        )
        .route(
            "/directories/:id/locations",
            get(services_locations::list_locations).post(services_locations::create_location),
        )
        .route(
            "/directories/:id/locations/:loc_id",
            put(services_locations::update_location).delete(services_locations::delete_location),
        )
        .route(
            "/directories/:id/services/import",
            post(services_locations::csv_import),
        )
        .route(
            "/directories/:id/locations/import",
            post(services_locations::csv_import),
        )
        .route(
            "/directories/:id/programmatic-pages",
            get(content_seo::list_programmatic_pages),
        )
        .route(
            "/directories/:id/programmatic-pages/:page_id",
            get(content_seo::get_programmatic_page),
        )
        .route(
            "/directories/:id/programmatic-pages/generate",
            post(content_seo::generate_programmatic_pages),
        )
        .route(
            "/directories/:id/programmatic-pages/bulk-status",
            post(content_seo::bulk_update_page_status),
        )
        // ??? Trap Door hyper-niche SEO pages
        .route(
            "/directories/:id/trap-doors/generate",
            post(trap_doors::generate_trap_doors),
        )
        .route(
            "/directories/:id/trap-doors/preview",
            post(trap_doors::preview_trap_doors),
        )
        .route(
            "/directories/:id/trap-doors/available-factors",
            get(trap_doors::available_factors),
        )
        .route(
            "/directories/:id/trap-doors/pages",
            get(trap_doors::list_trap_door_pages),
        )
        .route(
            "/directories/:id/trap-doors/analytics",
            get(trap_doors::trap_door_analytics),
        )
        .route(
            "/directories/:id/trap-doors/scheduled-generate",
            post(trap_doors::scheduled_generate_trap_doors),
        )
        .route(
            "/programmatic-pages/:page_id/track",
            post(trap_doors::track_page_event),
        )
        .route(
            "/directories/:id/topics",
            get(content_seo::list_topics).post(content_seo::create_topic),
        )
        .route(
            "/directories/:id/topics/:topic_id",
            put(content_seo::update_topic).delete(content_seo::delete_topic),
        )
        .route(
            "/directories/:id/topics/bulk",
            post(content_seo::bulk_topic_action),
        )
        .route(
            "/directories/:id/topics/suggestions",
            get(content_seo::suggest_topics),
        )
        .route(
            "/directories/:id/authors",
            get(content_seo::list_authors).post(content_seo::create_author),
        )
        .route(
            "/directories/:id/authors/:author_id",
            put(content_seo::update_author).delete(content_seo::delete_author),
        )
        .route(
            "/directories/:id/ai-draft",
            post(content_seo::generate_ai_draft),
        )
        .route(
            "/directories/:id/repurpose",
            post(content_seo::repurpose_content),
        )
        .route(
            "/directories/:id/internal-links",
            get(content_seo::internal_link_suggestions),
        )
        .route(
            "/directories/:id/seo-fallbacks",
            get(seo_config::list_seo_fallbacks),
        )
        .route(
            "/directories/:id/seo-fallbacks/:page_type",
            put(seo_config::upsert_seo_fallback),
        )
        .route(
            "/directories/:id/schema-configs",
            get(seo_config::list_schema_configs),
        )
        .route(
            "/directories/:id/schema-configs/:schema_type",
            put(seo_config::upsert_schema_config),
        )
        .route(
            "/directories/:id/seo-settings",
            get(seo_config::get_dir_seo_settings).put(seo_config::update_dir_seo_settings),
        )
        .route(
            "/directories/:id/sitemap",
            get(seo_config::generate_sitemap),
        )
        ...
        .route(
            ...
            post(branding::upload_branding_asset),
        )
        .route(
        ...
        .route(
            "/visitors/business/:business_id",
            get(visitors::business_visitor_summary),
        )
        .route(
            "/visitors/business/:business_id/events",
            get(visitors::business_visitor_events),
        )
        .route("/visitor/favorites", get(visitors::list_favorites))
        .route(
            "/visitor/favorites/check/:business_id",
            get(visitors::check_favorite),
        )
        .route(
        ...
        .route(
            "/bookmarks/count/:business_id",
            get(visitors::get_bookmark_count),
        )
        ...
        .route(
            "/connected-services",
            get(connected_services::list_connected_services),
        )
        .route(
            "/connected-services/connect",
            post(connected_services::connect_service),
        )
        .route(
            "/connected-services/verify",
            post(connected_services::verify_service_key),
        )
        .route(
            "/connected-services/coreswift/check",
            get(connected_services::check_coreswift_connection),
        )
        .route(
        ...
        .route(
            ...
            get(event_providers::list_providers).post(event_providers::create_provider),
        )
        .route(
            "/admin/directories/:directory_id/event-providers/:provider_id",
            put(event_providers::update_provider).delete(event_providers::delete_provider),
        )
        .route(
            "/admin/event-providers/:provider_id/test",
            post(event_providers::test_provider),
        )
        .route(
            "/admin/event-providers/:provider_id/sync",
            post(event_providers::sync_provider),
        )
        .route(
        ...
        .route(
            "/directories/:id/features",
            get(portal::get_directory_features).put(portal::update_directory_features),
        )
        ...
        .route(
            "/city-requests",
            get(visitors::get_city_requests).post(visitors::request_city),
        )
        ...
        .route(
            "/zaarhub/business/:slug/:id",
            get(zaarhub::get_business_detail),
        )
        .route("/zaarhub/deals", get(zaarhub::list_featured_deals))
        .route("/zaarhub/events", get(zaarhub::list_featured_events))
        .route(
            "/spotlight/:id/feature",
            post(zaarhub::toggle_spotlight_featured),
        )
        ...
        .route(
            "/zaarhub/editors-picks",
            get(zaarhub_cities::list_editors_picks),
        )
        .route(
            "/zaarhub/admin/listings/:id/editors-pick",
            patch(zaarhub_cities::toggle_editors_pick),
        )
        // ??? ZaarHub Claim/Redemption endpoints (public, Phase 5)
        .route(
            "/zaarhub/listings/:id/offers",
            get(zaarhub_cities::listing_offers),
        )
        .route("/zaarhub/offers/:id", get(zaarhub_cities::get_offer))
        .route(
            "/zaarhub/offers/:id/claim",
            post(zaarhub_cities::claim_offer),
        )
        .route(
            "/zaarhub/claims/:visitor_id",
            get(zaarhub_cities::visitor_claims),
        )
        // ??? ZaarHub Analytics (public read-only)
        .route(
            "/zaarhub/analytics/overview",
            get(zaarhub_analytics::overview),
        )
        .route(
            "/zaarhub/analytics/cities",
            get(zaarhub_analytics::city_performance),
        )
        .route(
            "/zaarhub/analytics/offers",
            get(zaarhub_analytics::top_offers),
        )
        .route(
            "/zaarhub/analytics/claims",
            get(zaarhub_analytics::recent_claims),
        )
        .route(
        ...
        .route(
            ...
            get(monetization::get_active_ads),
        )
        // ??? Public Spotlight & Notifications endpoints (Phase 4)
        .route(
            "/spotlight/:directory_id",
            get(monetization::get_spotlight_businesses),
        )
        .route(
            "/notifications/:directory_id",
            get(monetization::get_active_notifications),
        )
        ...
        .route(
            ...
            get(bookings::get_available_slots),
        )
        .route(
        ...
        .route(
            "/bookings",
            post(bookings::create_service_booking).get(bookings::list_visitor_bookings),
        )
        .route("/bookings/:id", get(bookings::get_booking))
        .route(
            "/bookings/:id/status",
            post(bookings::update_booking_status),
        )
        .route("/bookings/:id/cancel", post(bookings::cancel_booking))
        .route(
            "/business/:business_id/bookings",
            get(bookings::list_business_bookings),
        )
        // ? Stage 5: Service Catalog (business services/products)
        .route(
            "/services",
            get(service_catalog::list_services).post(service_catalog::create_service),
        )
        .route(
            "/services/:id",
            get(service_catalog::get_service)
                .put(service_catalog::update_service)
                .delete(service_catalog::delete_service),
        )
        .route(
            "/businesses/:business_id/services",
            get(service_catalog::list_services_for_business),
        )
        ...
        .route(
            "/pricing/bundles",
            get(pricing::list_bundles).post(pricing::create_bundle),
        )
        .route(
            "/pricing/bundles/:id",
            get(pricing::get_bundle)
                .put(pricing::update_bundle)
                .delete(pricing::delete_bundle),
        )
        .route("/pricing/grandfather", post(pricing::set_grandfathered))
        .route(
            "/pricing/grandfather/:business_id",
            get(pricing::get_grandfathered),
        )
        ...
        .route(
            "/admin/content-queue",
            get(content_queue::list_queue).post(content_queue::add_job),
        )
        .route(
            "/admin/content-queue/:id",
            put(content_queue::update_job).delete(content_queue::cancel_job),
        )
        .route(
        ...
        .route(
            "/admin/tag-rules",
            get(tag_automation::list_rules).post(tag_automation::create_rule),
        )
        .route(
            "/admin/tag-rules/:id",
            put(tag_automation::update_rule).delete(tag_automation::delete_rule),
        )
        .route(
            "/admin/tag-rules/execute",
            post(tag_automation::execute_rules_for_contact),
        )
        .route(
            "/admin/tracked-links",
            get(tag_automation::list_tracked_links).post(tag_automation::create_tracked_link),
        )
        .route(
            "/admin/tracked-links/:id",
            put(tag_automation::update_tracked_link).delete(tag_automation::delete_tracked_link),
        )
        .route(
            "/admin/tracked-links/bulk",
            post(tag_automation::bulk_create_tracked_links),
        )
        .route(
            "/admin/tracked-links/stats/:id",
            get(tag_automation::get_link_stats),
        )
        // ??? Onboarding Survey admin endpoints
        .route(
            "/admin/directories/:id/survey",
            get(onboarding_survey::get_survey_config).put(onboarding_survey::upsert_survey_config),
        )
        .route(
        ...
        .route(
            "/admin/directories/:id/city-requests",
            get(visitors::admin_get_city_requests),
        )
        .route(
            "/admin/directories/:id/city-requests/:request_id/mark-added",
            post(visitors::admin_mark_city_added),
        )
        // ? Business messaging - owner routes (auth required)
        .route("/messages/:business_id", get(messaging::list_messages))
        .route(
            "/messages/:business_id/unread",
    ...
    let login_html = std::fs::read_to_string(&login_path).unwrap_or_else(|_| index_html.clone());
                ...
                async move {
                    ...
                    if let Some(ref host) = host {
                        ...
                        let is_app = host == &app_domain
                            || host == &www_domain
                            || host == "localhost"
                            || host == "directory.swiftsoftware.net"
                    ...
                    if path == "/admin/pricing" || path.starts_with("/admin/pricing/") {
                        let pricing_path =
                            std::path::Path::new(&frontend).join("pricing-admin.html");
                    ...
                    if path == "/blog-features" || path == "/blog-features/" {
                        let blog_feat_path =
                            std::path::Path::new(&frontend).join("blog-features-admin.html");
                    ...
                    if path == "/research" || path == "/research/" {
                        let research_path =
                            std::path::Path::new(&frontend).join("content-research.html");
                    ...
                    if path == "/portal"
                        || path == "/portal/"
                        || path.starts_with("/portal/business")
                    {
                        let portal_path =
                            std::path::Path::new(&frontend).join("business-portal.html");
                    ...
                    if path == "/visitor"
                        || path == "/visitor/"
                        || path.starts_with("/visitor/portal")
                    {
                        let portal_path =
                            std::path::Path::new(&frontend).join("visitor-portal.html");
                        if portal_path.exists() {
                            match tokio::fs::read(&portal_path).await {
                                Ok(content) => {
                                    return Ok::<_, std::convert::Infallible>(
                    ...
                    if path == "/hotel-savings/faq" {
                        let faq_path =
                            std::path::Path::new(&frontend).join("hotel-savings-faq.html");
                        if faq_path.exists() {
                    ...
                    if path == "/hotel-savings/terms" {
                        let terms_path =
                            std::path::Path::new(&frontend).join("hotel-savings-terms.html");
                    ...
                    if path == "/supplier"
                        ...
                        || path == "/distributor"
                        || path == "/distributor/"
                        || path.starts_with("/distributor/dashboard")
                    {
                        let portal_path =
                            std::path::Path::new(&frontend).join("supplier-portal.html");
                        if portal_path.exists() {
                            match tokio::fs::read(&portal_path).await {
                                Ok(content) => {
                                    return Ok::<_, std::convert::Infallible>(
                    ...
                    {
                        let dir_path =
                            std::path::Path::new(&frontend).join("supplier-directory.html");
                        if dir_path.exists() {
                    ...
                    if path.starts_with("/biz/") {
                        let detail_path =
                            std::path::Path::new(&frontend).join("business-detail.html");
                    ...
                    // SPA fallback: serve full index.html for all unmatched routes
                    {
                        ...
                        let branding = if let Some(slug) = slug {
                            crate::branding_injector::fetch_branding_by_slug(&_pool_for_host, &slug)
                                .await
                        } else {
                            None
                        };
...
async fn auth_guard(
    ...
    let auth_preview = req
        ...
        .collect::<String>();
    warn!(
        "AUTH_GUARD: path={}, has_auth={}, auth_preview={}",
        path, has_auth, auth_preview
    );
