...
fn extract_claims_from_headers(headers: &HeaderMap, jwt_secret: &str) -> Result<Claims, AppError> {
    ...
    crate::auth::middleware::verify_token(token, jwt_secret).map_err(|_| AppError::Unauthorized)
}
...
pub async fn get_available_slots(
    ...
    let tenant_id = tenant_id.ok_or(AppError::BadRequest(format!(
...
pub async fn create_booking(
    ...
    if req.preferred_date.trim().is_empty() {
        return Err(AppError::Validation(
            "preferred_date is required (YYYY-MM-DD)".to_string(),
        ));
    }
    ...
    let (business_id, business_name, business_website) =
    ...
    tracing::info!(
        "[bookings] CoreSwift checkout response: status={}, body={}",
        status,
        body_text
    );
...
async fn ensure_calendar_exists(
    ...
    if !cal_status.is_success() {
        let body_text = calendar_resp
            .text()
            .await
            .unwrap_or_else(|_| "unknown".to_string());
        if !body_text.contains("already exists")
            && !body_text.contains("duplicate key")
            && !body_text.contains("unique")
        {
            return Err(format!(
                "CoreSwift calendar creation returned {}: {}",
                cal_status, body_text
            ));
        }
    }
    ...
    if slot_status.is_success() || slot_status.as_u16() == 409 || slot_status.as_u16() == 422 {
        ...
        let body_text = slot_resp
            .text()
            .await
            .unwrap_or_else(|_| "unknown".to_string());
        tracing::warn!(
            "[bookings] Default slot creation non-fatal: {} {}",
            slot_status,
            body_text
        );
        Ok(())
...
async fn advance_deal_to_qualified(db: &sqlx::PgPool, business_id: Uuid) -> Result<(), String> {
    ...
    if let Some(deal_id) = deal_id {
        sqlx::query(
            "UPDATE crm_deal_records SET stage = 'Qualified', updated_at = NOW() WHERE id = $1",
        )
        .bind(deal_id)
        .execute(db)
        .await
...
pub async fn create_service_booking(
    ...
    if !biz_exists {
        return Err(AppError::NotFound(
            "Business not found in this directory".to_string(),
        ));
    }
...
pub async fn list_business_bookings(
    ...
    if !is_authorized {
        return Err(AppError::Forbidden(
            "Not authorized to view bookings for this business".to_string(),
        ));
    }
    ...
    Ok(Json(
        json!({ "success": true, "bookings": bookings, "business_id": business_id }),
    ))
}
...
pub async fn update_booking_status(
    ...
    let biz_id: Uuid = sqlx::query_scalar("SELECT business_id FROM service_bookings WHERE id = $1")
        .bind(booking_id)
        .fetch_optional(&s.db)
        .await?
        .ok_or(AppError::NotFound("Booking not found".to_string()))?;
    ...
    if !is_authorized {
        return Err(AppError::Forbidden(
            "Not authorized to update this booking's status".to_string(),
        ));
    }
    ...
    let current_status: String =
        sqlx::query_scalar("SELECT status FROM service_bookings WHERE id = $1")
            .bind(booking_id)
            .fetch_one(&s.db)
            .await?;
    ...
    if current_status == "completed" || current_status == "cancelled" {
        return Err(AppError::BadRequest(format!(
            "Cannot update status of a {} booking",
            current_status
        )));
    }
...
pub async fn cancel_booking(
    ...
    let booking: Option<(Uuid, String)> =
        sqlx::query_as("SELECT visitor_account_id, status FROM service_bookings WHERE id = $1")
            .bind(booking_id)
            .fetch_optional(&s.db)
            .await?;

    let (owner_id, current_status) =
...
async fn notify_business_owners(
    ...
    if let Some(owner_emails) = get_business_owner_emails(&s.db, business_id).await {
        for owner_email in &owner_emails {
            let _ =
                send_booking_notification_email(s, owner_email, business_id, service_name).await;
        }
    }
