...
pub async fn import_data(
    ...
    if req.data.is_empty() {
        return Err(AppError::Validation(
            "No data provided for import".to_string(),
        ));
    }
...
async fn import_business_row(
    ...
    let name = row
        .get("name")
        .and_then(|v| v.as_str())
        .ok_or_else(|| AppError::Validation("Missing 'name' field".to_string()))?;
    let slug = row.get("slug").and_then(|v| v.as_str()).unwrap_or(name);
    let directory_id = row
        .get("directory_id")
        .and_then(|v| v.as_str())
...
async fn import_review_row(
    ...
    let reviewer_name = row
        .get("reviewer_name")
        .or_else(|| row.get("author"))
        .and_then(|v| v.as_str())
        .unwrap_or("Anonymous");
    sqlx::query(
...
async fn import_deal_row(
    ...
    let directory_id = row
        ...
        .or(dir_id)
        .ok_or_else(|| AppError::Validation("Missing 'directory_id' field".to_string()))?;
    sqlx::query("INSERT INTO deals (title, description, directory_id) VALUES ($1, $2, $3)")
        .bind(title)
        .bind(description)
        .bind(directory_id)
        .execute(db)
...
pub async fn enrich_business_from_sources(
    ...
    if source_filter == "google" || source_filter == "both" {
        match enrich_from_google(&state, &req).await {
            ...
            Ok(None) => { /* no results */ }
            Err(e) => {
                sources_checked.push(format!("google_error: {}", e));
            }
        }
    }
    ...
    if source_filter == "yelp" || source_filter == "both" {
        match enrich_from_yelp(&state, &req).await {
            ...
            Ok(None) => { /* no results */ }
            Err(e) => {
                sources_checked.push(format!("yelp_error: {}", e));
            }
        }
    }
    ...
    result
        .as_object_mut()
        .unwrap()
        .insert("sources_checked".to_string(), json!(sources_checked));
    Ok(Json(result))
}

async fn enrich_from_google(
    state: &AppState,
    req: &EnrichRequest,
) -> Result<Option<serde_json::Value>, String> {
    let api_key = get_google_api_key(state, Some(&req.directory_id))
        .await
    ...
    let place_id = predictions.and_then(|arr| arr.first()).and_then(|p| {
        p.get("place_id")
    ...
    if let Some(pid) = place_id {
        ...
        let dbody: serde_json::Value = dresp
            .json()
            .await
...
async fn enrich_from_yelp(
    state: &AppState,
    req: &EnrichRequest,
) -> Result<Option<serde_json::Value>, String> {
    let api_key = get_yelp_api_key(state, Some(&req.directory_id))
        .await
...
fn merge_enrichment(target: &mut serde_json::Value, source: serde_json::Value) {
    ...
    if let Some(src_obj) = source.as_object() {
        ...
        if let Some(src_images) = src_obj.get("images").and_then(|v| v.as_array()) {
            let mut existing: HashSet<String> = target_obj
                .get("images")
                .and_then(|v| v.as_array())
                .map(|arr| {
                    arr.iter()
...
pub async fn list_import_logs(State(state): State<AppState>) -> ApiResult<impl IntoResponse> {
...
pub async fn get_import_log(
    ...
    let log = sqlx::query_as::<_, ImportLog>("SELECT * FROM import_logs WHERE id = $1")
        .bind(id)
...
pub async fn list_export_templates(State(state): State<AppState>) -> ApiResult<impl IntoResponse> {
    let templates =
        sqlx::query_as::<_, ExportTemplate>("SELECT * FROM export_templates ORDER BY name")
            .fetch_all(&state.db)
            .await?;
...
pub async fn get_export_template(
    ...
    let template =
        sqlx::query_as::<_, ExportTemplate>("SELECT * FROM export_templates WHERE id = $1")
            .bind(id)
            .fetch_optional(&state.db)
            .await?
...
pub async fn run_export_template(
    ...
    let data: Vec<serde_json::Value> = match template.entity_type.as_str() {
        ...
        "reviews" => {
            let rows = sqlx::query(
                "SELECT row_to_json(r.*)::text as j FROM reviews r ORDER BY r.created_at",
            )
            .fetch_all(&state.db)
            .await?;
            rows.iter()
        ...
        "contacts" => {
            let rows = if let Some(did) = template.directory_id {
                ...
                sqlx::query(
                    "SELECT row_to_json(c.*)::text as j FROM crm_contacts c ORDER BY c.first_name",
                )
                .fetch_all(&state.db)
                .await?
            };
