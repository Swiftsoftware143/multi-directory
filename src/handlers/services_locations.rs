...
pub async fn list_services(
    ...
    Ok(Json(
        sqlx::query_as::<_, DirectoryService>(
...
pub async fn update_service(
    State(s): State<AppState>,
    Path((dir_id, svc_id)): Path<(Uuid, Uuid)>,
    Json(req): Json<UpdateServiceReq>,
) -> ApiResult<impl IntoResponse> {
    sqlx::query_as::<_, DirectoryService>(
        "SELECT * FROM directory_services WHERE id=$1 AND directory_id=$2",
    )
    .bind(svc_id)
    .bind(dir_id)
    .fetch_optional(&s.db)
    .await?
    .ok_or(AppError::NotFound("Service".into()))?;
    let svc = sqlx::query_as::<_, DirectoryService>(
...
pub async fn delete_service(
    ...
    let r = sqlx::query("DELETE FROM directory_services WHERE id=$1 AND directory_id=$2")
        .bind(svc_id)
        .bind(dir_id)
        .execute(&s.db)
        .await?;
...
pub async fn list_locations(
    ...
    Ok(Json(
        sqlx::query_as::<_, DirectoryLocation>(
...
pub async fn update_location(
    State(s): State<AppState>,
    Path((dir_id, loc_id)): Path<(Uuid, Uuid)>,
    Json(req): Json<UpdateLocationReq>,
) -> ApiResult<impl IntoResponse> {
    sqlx::query_as::<_, DirectoryLocation>(
        "SELECT * FROM directory_locations WHERE id=$1 AND directory_id=$2",
    )
    .bind(loc_id)
    .bind(dir_id)
    .fetch_optional(&s.db)
    .await?
    .ok_or(AppError::NotFound("Location".into()))?;
    let loc = sqlx::query_as::<_, DirectoryLocation>(
...
pub async fn delete_location(
    ...
    let r = sqlx::query("DELETE FROM directory_locations WHERE id=$1 AND directory_id=$2")
        .bind(loc_id)
        .bind(dir_id)
        .execute(&s.db)
        .await?;
...
pub async fn csv_import(
    ...
    for row in &req.rows {
        ...
        if req.target == "services" {
            let desc = row.get(2);
            let ex: Option<(i64,)> = sqlx::query_as(
                "SELECT COUNT(*) FROM directory_services WHERE directory_id=$1 AND slug=$2",
            )
            .bind(dir_id)
            .bind(&slug)
            .fetch_optional(&s.db)
            .await?;
            if ex.map(|e| e.0).unwrap_or(0) > 0 {
                skipped += 1;
                continue;
            }
            ...
            let ex: Option<(i64,)> = sqlx::query_as(
                "SELECT COUNT(*) FROM directory_locations WHERE directory_id=$1 AND slug=$2",
            )
            .bind(dir_id)
            .bind(&slug)
            .fetch_optional(&s.db)
